...
const checkout = jest
  .fn()
  .mockResolvedValue({ url: 'https://checkout.example.test/session' });
...
describe('Application Florésia : commande et paiement via HTTP', () => {
  ...
  const products = [
    { id: 'p', name: 'Romance', price: 32 },
    { id: CUSTOM_BOUQUET_PRODUCT_ID, name: 'Bouquet personnalisé', price: 0 },
  ];
  const flowers = [
    {
      id: 'rose',
  ...
  const prisma = {
    user: {
      findUnique: jest.fn(() => ({ id: 'u', isEmailVerified: emailVerified })),
    },
        ...
        const order: Order = {
          ...
          items: data.items.create.map((item) => ({
            ...
            product: {
              ...
              name: products.find((p) => p.id === item.product.connect.id)!
                .name,
            },
          })),
        };
  ...
  const config: Record<string, string> = {
    JWT_SECRET: 'test-only-secret-for-http-integration',
    JWT_EXPIRES_IN: '1h',
    STRIPE_SECRET_KEY: 'sk_test_local_only',
    STRIPE_WEBHOOK_SECRET: 'whsec_test_local_only',
    ...
    const module = await Test.createTestingModule({ imports: [AppModule] })
      ...
      .useValue({
        ...
        getOrThrow: (key: string) => {
          if (!(key in config)) throw new Error('Missing test config: ' + key);
          return config[key];
        },
      })
      ...
      .useValue({
        sendMessage: jest.fn().mockResolvedValue({ reply: 'Réponse de test' }),
      })
      .compile();
  ...
  const standard = {
    addressId: 'addr',
    items: [{ productId: 'p', quantity: 1, unitPrice: 0.01 }],
    totalAmount: 0.01,
  };
  ...
  it('refuse une quantité fractionnaire avant la création', async () => {
    await request(app.getHttpServer())
      .post('/orders')
      .set('Authorization', 'Bearer ' + token)
      .send({ ...standard, items: [{ productId: 'p', quantity: 1.5 }] })
      .expect(400);
  ...
  it('recalcule le prix, transmet les frais à Stripe puis confirme uniquement le montant attendu', async () => {
    ...
    expect(checkout).toHaveBeenCalledWith(
      expect.objectContaining({
        line_items: [
          expect.objectContaining({
            price_data: expect.objectContaining({ unit_amount: 3200 }),
            quantity: 1,
          }),
          expect.objectContaining({
            price_data: expect.objectContaining({ unit_amount: 590 }),
            quantity: 1,
          }),
        ],
      }),
  ...
  it('calcule une composition à la tige et conserve le détail du panier', async () => {
    ...
    expect(response.body.totalAmount).toBe(18.4);
  });
