...
(async () => {
  ...
  await prisma.$transaction(async tx => {
    ...
    await tx.flower.updateMany({ where: { name: 'Rose', color: 'Rose' }, data: { imageUrl: '/floresia-app/flowers/rose-pink.png', isSecondary: false } });
    const lilac = await tx.flower.findFirst({ where: { name: 'Rose', color: 'Lilas' } });
    if (lilac) {
      await tx.flower.updateMany({ where: { name: 'Rose', color: 'Lilas' }, data: { imageUrl: '/floresia-app/flowers/rose-lilas.png', isSecondary: false } });
    } else {
      await tx.flower.create({ data: { name: 'Rose', color: 'Lilas', description: 'Élégance et enchantement', imageUrl: '/floresia-app/flowers/rose-lilas.png', price: pink.price, stock: pink.stock, isSecondary: false } });
    }
