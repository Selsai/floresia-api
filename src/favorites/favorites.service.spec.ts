...
describe('FavoritesService : favoris privés et doublons', () => {
  ...
  beforeEach(() => {
    ...
    prisma.product.findUnique.mockResolvedValue({ id: 'p' });
  });
  ...
  it('refuse un produit absent', async () => {
    ...
    await expect(service.add('u', 'p')).rejects.toBeInstanceOf(
      NotFoundException,
    );
  ...
  it('ne crée pas deux fois un favori existant', async () => {
    ...
    expect(await service.add('u', 'p')).toEqual(existing);
    expect(prisma.favorite.create).not.toHaveBeenCalled();
  });
  it('crée un favori associé au compte et au produit', async () => {
    await service.add('u', 'p');
  ...
  it('refuse de supprimer un favori absent pour ce compte', async () => {
    await expect(service.remove('u', 'p')).rejects.toBeInstanceOf(
      NotFoundException,
    );
  ...
  it('supprime uniquement le favori trouvé pour le compte et le produit', async () => {
    prisma.favorite.findUnique.mockResolvedValue({ id: 'f' });
    await service.remove('u', 'p');
    expect(prisma.favorite.findUnique).toHaveBeenCalledWith({
      where: { userId_productId: { userId: 'u', productId: 'p' } },
    });
