...
describe('ChatbotService : contexte et erreurs', () => {
  ...
  it.each([
    ...
    await service.sendMessage(
      { message: 'Quelles fleurs en ce moment ?' },
      instant,
    );
  ...
  it('conserve le contexte réel malgré une ancienne réponse erronée', async () => {
    ...
    expect(request.config.systemInstruction).toContain(
      "Une erreur dans l'historique doit être corrigée",
    );
    expect(request.config.systemInstruction).toContain(
      'Bouquet témoin (Autre, 45€)',
    );
  ...
  it('répond au prix depuis la base sans utiliser une réponse inventée par Gemini', async () => {
    generateContent.mockResolvedValue({
      text: 'Le bouquet Inventé coûte 99 €.',
    });
    await expect(
      service.sendMessage(
        {
      ...
      reply: 'Le bouquet « Bouquet témoin » coûte 45,00 €, hors livraison.',
    });
    expect(generateContent).not.toHaveBeenCalled();
  });

  it('transmet aussi les feuillages et leurs prix à Gemini', async () => {
    findFlowers.mockResolvedValue([
      {
        name: 'Eucalyptus',
        color: 'Vert',
        price: 2,
    ...
    await service.sendMessage(
      { message: 'Comment entretenir mon eucalyptus ?' },
      'secondary-context',
    );
    expect(findFlowers).toHaveBeenCalledWith(
      expect.objectContaining({ where: { stock: { gt: 0 } } }),
    );
    expect(generateContent.mock.calls[0][0].config.systemInstruction).toContain(
      'Eucalyptus (complément optionnel) : Vert (2€) par tige, sans lot',
    );
  });

  it('inclut les vrais bouquets personnalisables en excluant seulement le produit interne', async () => {
    await service.sendMessage(
      { message: 'Quel bouquet choisir ?' },
      'real-catalog',
    );
  ...
  it('passe directement au modèle de secours si le quota du premier est épuisé', async () => {
    ...
    await expect(
      service.sendMessage({ message: 'Bonjour' }, 'quota-fallback'),
