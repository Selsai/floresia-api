...
export function catalogReply(
  ...
  if (/\b(prix|coute|cout|tarif|combien)\b/.test(question)) {
    const namedProducts = products.filter(product => {
      ...
      const explicitAlias = new RegExp(`\\b(?:prix|bouquet|coute|tarif)\\s*(?:(?:du|de|le|la|ce|pour|des|un)\\s+){0,3}[«"“:]?\\s*${escapedName}\\b`);
      return question.includes(fullName) || (shortName.length >= 4 && explicitAlias.test(question));
    });
    if (namedProducts.length) {
      return namedProducts.map(product => `Le bouquet « ${product.name} » coûte ${euros(product.price)}, hors livraison.`).join(' ');
    }
  }

  if (/\b(total|combien|revient|cout)\b/.test(question) && !/\b(remise|reduction|pourcent)\b|%/.test(question)) {
    const quantities: Record<string, number> = { un: 1, une: 1, deux: 2, trois: 3, quatre: 4, cinq: 5, six: 6, sept: 7, huit: 8, neuf: 9, dix: 10 };
    const terms = [...question.matchAll(/\b(\d+|un|une|deux|trois|quatre|cinq|six|sept|huit|neuf|dix)\s+([^€;]+?)\s+a\s+(\d+(?:[.,]\d{1,2})?)\s*(?:€|euros?)/g)]
      .map(match => ({ quantity: quantities[match[1]] ?? Number(match[1]), cents: Math.round(Number(match[3].replace(',', '.')) * 100) }));
    if (terms.length >= 2 && terms.every(term => term.quantity > 0)) {
      const total = terms.reduce((sum, term) => sum + term.quantity * term.cents, 0);
      const details = terms.map(term => `${term.quantity} × ${euros(term.cents / 100)}`).join(' + ');
  ...
  if (/\bsans\s+(?:(?:du|de|aucun)\s+)?feuillages?\b/.test(question) && /\b(bouquet|composer|personnal)/.test(question)) {
  ...
  if (/\bcomment\b/.test(question) && /\b(creer|composer|faire|personnaliser)\b/.test(question) && /personnal/.test(question)) {
  ...
  const mainFlowers = flowers.filter(flower => !flower.isSecondary && flower.stock > 0);
  if (/\bbouquets?\b/.test(question) && budget !== undefined && budget >= 0 && mainFlowers.length &&
    !products.some(product => product.price <= budget) && !mainFlowers.some(flower => flower.price <= budget)) {
  ...
  if (occasion && /\bbouquets?\b/.test(question) && /recommand|conseill|propos|quel/.test(question) &&
    !/\b(saison|blanc|blanche|blanches|blancs|rouge|rouges|rose|roses|parfum|sans|composition|compose|personnalise)\b/.test(question)) {
    const choices = products.filter(product => product.category === occasion[1] && (budget === undefined || product.price <= budget)).slice(0, 4);
    if (!choices.length) {
