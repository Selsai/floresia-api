...
describe('ProductsService : catalogue et erreurs', () => {
  ...
  beforeEach(() => { jest.resetAllMocks(); service = new ProductsService({ product: model } as unknown as PrismaService); model.findUnique.mockResolvedValue({ id: 'p' }); });
  ...
  it('conserve les données validées du catalogue lors de la création', async () => {
    const dto = { name: 'Test', price: 3.5 } as CreateProductDto; await service.create(dto);
