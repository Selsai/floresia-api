...
describe('OrdersController : contrats HTTP et droits', () => {
  ...
  beforeAll(async () => { fixture = await createHttpFixture(OrdersController, OrdersService, service); });
  ...
  it('applique les droits du compte utilisateur', async () => {
    await request(fixture.app.getHttpServer()).delete('/orders/p').set('Authorization', 'Bearer ' + fixture.userToken).expect(403);
    expect(service.remove).not.toHaveBeenCalled();
  });
  it('autorise la suppression par un administrateur', async () => {
    await request(fixture.app.getHttpServer()).delete('/orders/p').set('Authorization', 'Bearer ' + fixture.adminToken).expect(200);
