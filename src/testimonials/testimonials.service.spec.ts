...
describe('TestimonialsService : publication et propriété', () => {
  ...
  beforeEach(() => { jest.resetAllMocks(); service = new TestimonialsService({ testimonial: model, user } as unknown as PrismaService); model.findUnique.mockResolvedValue({ id: 'p', authorId: 'u' }); user.findUnique.mockResolvedValue({ id: 'u', firstName: 'Flora', lastName: 'Test', isEmailVerified: true }); });
  ...
  it('enregistre les données validées par l’administration', async () => {
    const dto = {} as CreateTestimonialDto; await service.create(dto); expect(model.create).toHaveBeenCalledWith({ data: dto });
  });
  ...
  it('refuse une publication avec un compte absent', async () => {
    user.findUnique.mockResolvedValue(null); await expect(service.submit('u', { content: 'Très beau bouquet', rating: 5 })).rejects.toBeInstanceOf(NotFoundException);
  });
  it('refuse une publication avant vérification de l’email', async () => {
    user.findUnique.mockResolvedValue({ isEmailVerified: false }); await expect(service.submit('u', { content: 'Très beau bouquet', rating: 5 })).rejects.toBeInstanceOf(ForbiddenException); expect(model.create).not.toHaveBeenCalled();
  });
  it('associe les données publiées au compte vérifié', async () => {
    await service.submit('u', { content: 'Très beau bouquet', rating: 5 }); expect(model.create).toHaveBeenCalledWith({ data: { authorName: 'Flora T.', content: 'Très beau bouquet', rating: 5, occasion: undefined, authorId: 'u' } });
  });
