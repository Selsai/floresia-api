...
describe('Classification des fleurs : validation HTTP', () => {
  ...
  it.each([true, false])(
    'accepte isSecondary=%s dans un PATCH partiel',
    async (value) => {
      await expect(
        pipe.transform({ isSecondary: value }, metadata),
  ...
  it('refuse une chaîne à la place du booléen', async () => {
    await expect(
      pipe.transform({ isSecondary: 'true' }, metadata),
    ).rejects.toThrow();
  });

  it('continue de refuser les propriétés inconnues', async () => {
    await expect(
      pipe.transform({ unexpected: true }, metadata),
    ).rejects.toThrow();
  });
});
