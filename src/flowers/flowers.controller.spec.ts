...
describe('FlowersController : contrats HTTP et droits', () => {
  ...
  beforeAll(async () => { fixture = await createHttpFixture(FlowersController, FlowersService, service); });
  afterAll(async () => { await fixture.app.close(); });
  beforeEach(() => { jest.clearAllMocks(); });
  it('rend la consultation publique accessible', async () => { await request(fixture.app.getHttpServer()).get('/flowers').expect(200); });
  ...
  it('applique les droits du compte utilisateur', async () => {
    await request(fixture.app.getHttpServer()).delete('/flowers/p').set('Authorization', 'Bearer ' + fixture.userToken).expect(403);
    expect(service.remove).not.toHaveBeenCalled();
  });
  it('autorise la suppression par un administrateur', async () => {
    await request(fixture.app.getHttpServer()).delete('/flowers/p').set('Authorization', 'Bearer ' + fixture.adminToken).expect(200);
